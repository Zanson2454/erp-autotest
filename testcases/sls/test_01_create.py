...
class TestSalesOrderCreate(BaseTest):
    ...
    def _render_order_line(self):
        ...
        logger.debug(f"订单行渲染请求URL: {url}")
        logger.debug(f"订单行渲染请求头: {self.headers}")
        ...
        try:
            ...
            logger.error(f"订单行渲染原始响应: {json.dumps(result, ensure_ascii=False, indent=2)}")
            raise
