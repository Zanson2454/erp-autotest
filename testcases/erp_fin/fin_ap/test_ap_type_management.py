...
class TestApTypeMdManagement(ApBaseTest):
    ...
    def test_query_ap_type_page(self):
        ...
        try:
            ...
            set_dict = {
                "pageable": {"pageNo": 1, "pageSize": 20, "sortOrders": [], "conditionItems": {}, "conditionGroup": {}}
            }

            # 2. 标准化API调用
            response, _ = self.standard_api_call(api_key="应付单类型配置表-分页数据服务_PmHKWs1", set_dict=set_dict)
    ...
    def test_save_ap_type(self):
        ...
        try:
            ...
            response, extracted_id = self.standard_api_call(
                api_key="应付单类型-保存应付单类型服务", set_dict=set_dict, store_id_as="ap_type"
            )
    ...
    def test_update_ap_type(self):
        ...
        try:
            ...
            response, _ = self.standard_api_call(api_key="应付单类型-保存应付单类型服务", set_dict=self.ap_type_detail)
    ...
    def test_enable_ap_type(self):
        ...
        try:
            ...
            response, _ = self.standard_api_call(api_key="应付单类型配置表-批量启用主数据服务", set_dict=set_dict)
    ...
    def test_disable_ap_type(self):
        ...
        try:
            ...
            # 2. 批量禁用数据（确保ID是整数类型）
            set_dict = {"ids": [int(self.ap_type_id)] if self.ap_type_id else []}
            response, _ = self.standard_api_call(api_key="应付单类型配置表-批量禁用主数据服务", set_dict=set_dict)
    ...
    @case_decorator(
        story="应付单据类型禁用",
        title="测试禁用应付单据类型",
        description="验证禁用应付单据类型功能",
        severity="normal",
        file_level_order=7,
        tags=["应付单据类型", "禁用"],
    )
    def test_single_disable_ap_type(self):
        """测试禁用应付单据类型"""
        try:
            # 1. 确保有数据
            if not self.ap_type_id:
    ...
    def test_batch_delete_ap_type(self):
        ...
        try:
            ...
            ap_type_ids = []
            for i in range(2):
                self.mock_util.get_timestamp()

                set_dict = {"ids": [int(self.ap_type_id)] if self.ap_type_id else []}
                response, _ = self.standard_api_call(api_key="应付单类型配置表-批量删除数据服务", set_dict=set_dict)
                self.assert_util.assert_response_success(response)
