...
class TestSettItemBusiCheck(FinBaseTest):
    ...
    @case_decorator(
        story="结算管理", title="批量任务记录", description="批量任务记录", severity="normal", file_level_order=2
    )
    def test_batch_task_record(self):
        ...
        try:
            ...
            params, url = self.get_api_params(api_path)
            filter_data = ParamUtil.filter_post_body_fields(
                params, ["pageNo", "pageSize", "conditionItems", "sortOrders"], ["params", "request", "pageable"]
            )
    ...
    def test_batch_get_scope(self, test_params):
        ...
        try:
            ...
            self.assert_util.assert_by_operator(
                result.get("data", {}).get("data", {}).get("docType", {}), "=", test_params["docType"]
            )
            self.assert_util.assert_by_operator(
                result.get("data", {}).get("data", {}).get("operType", {}), "=", test_params["operType"]
            )
            self.assert_util.assert_by_operator(
                result.get("data", {}).get("data", {}).get("taskStatus", {}), "=", test_params["expected_status"]
            )
    ...
    def test_find_sett_doc_by_id(self):
        ...
        try:
            ...
            self.assert_util.assert_response_success(result)
            self.assert_util.assert_by_operator(
                result.get("data", {}).get("data", {}).get("taskCode", {}), "=", task_code
            )
            a.json(data, "请求数据")
            a.json(result, "响应数据")
        except Exception as e:
    ...
    def test_cancal_scope(self):
        ...
        try:
            ...
            for index, task_info in enumerate(task_list):
                data = ParamUtil.filter_post_body_fields(
                    params_base, ["docType", "id", "operType", "taskCode", "taskStatus"], ["params", "request"]
                )
