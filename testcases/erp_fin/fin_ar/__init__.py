...
@allure.feature("应收管理")
class ArBaseTest(QueryServiceMixin, YamlUtilMixin, MockUtilMixin, BaseTest):
    """应收单测试基类，提供通用功能"""
    ...
    def send_api_request(self, api_key, request_data, result_dict):
        ...
        filtered_params = ParamUtil.filter_post_body_fields(params, list(request_data.keys()), ["params", "request"])
    ...
    def query_ar_detail(self, ar_doc_id, ar_detail=None):
        """查询应收单详情；兼容旧签名，支持传入外部dict或直接返回详情。"""
        ...
        if isinstance(ar_detail, dict):
            ar_detail.update(detail)
        return detail
    ...
    def build_sb_request_params(self, sb_record, sb_id, bil_code, status, output_dict):
        ...
        output_dict.update({"core_fields": core_fields, "request_data": request_data})
