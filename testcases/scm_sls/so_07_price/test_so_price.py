...
class TestSoPrice(SlsBase):
                                ...
                                for where_condition, where_params in possible_conditions:
                                    try:
                                        result = cls.db.delete(
                                            table="gen_match_record_md", where=where_condition, params=where_params
                                        )
                                        if result > 0:
    ...
    def test_check_price_exists(self):
        ...
        try:
            ...
            if records:
                ...
                if found_mat:
                    a.text(
                        f"销售价格校验成功：物料 {self.fixed_mat_name}(ID: {self.fixed_mat_id}) 存在销售价格",
                        "校验结果",
                    )
                    # 保存价格记录供后续用例使用
                    ...
                    # 获取原价格并计算新价格
                    original_price = (
                        found_record.get("out1") or found_record.get("outNew1") or found_record.get("price")
                    )
                    if original_price:
                        try:
                    ...
                    if records and total > 0:
                        ...
                        for record in records:
                            ...
                            try:
                                ...
                                if mat_id == self.fixed_mat_id:
                                    ...
                                    # 获取原价格并计算新价格
                                    original_price = (
                                        found_record.get("out1")
                                        or found_record.get("outNew1")
                                        or found_record.get("price")
                                    )
                                    if original_price:
                                        try:
                                            original_price = float(original_price)
                        ...
                        raise ValueError("创建价格后，查询销售价格列表仍为空")
            else:
                # 如果没有记录，说明该物料不存在销售价格，先创建价格
                a.text(
                    f"销售价格校验结果：物料 {self.fixed_mat_name}(ID: {self.fixed_mat_id}) 不存在销售价格，开始创建价格",
                    "校验结果",
                )
    ...
    def _ensure_price_record_context(self):
        ...
        def _query_current_price_record():
            ...
            records = response.get("data", {}).get("data", {}).get("records", []) or response.get("data", {}).get(
                "data", {}
            ).get("data", [])
            current_time = self.mock_util.get_timestamp(timestamp=True)
            candidates = []
    ...
    def _create_and_submit_price(self):
        ...
        try:
            ...
            submit_filtered_params = ParamUtil.filter_post_body_fields(submit_params, ["id"], ["params", "request"])

            submit_set_dict = {"id": price_adj_id}
            ...
            response, _ = self.standard_api_call(
                ...
                set_dict=(
                    submit_filtered_params.get("params", {})
                    if isinstance(submit_filtered_params, dict)
                    else submit_filtered_params
                ),
                store_id_as=None,
                use_param_util=False,
                param_path=["params"],
            ...
            try:
                ...
                if price_adj_info:
                    status = (
                        price_adj_info.get("status")
                        or price_adj_info.get("doc_status")
                        or price_adj_info.get("biz_status")
                    )
                    if status:
                        ...
                        a.text("价格维护单提交成功，但状态字段为空", "创建价格")
                else:
                    a.text("价格维护单提交成功，但无法查询状态", "创建价格")
            except Exception as e:
    ...
    def test_create_order_and_verify_price(self):
    ...
    def _maintain_price(self, price_record, original_price, new_price):
        ...
        try:
            ...
            if isinstance(var9, dict):
                ...
                mat_info = {"matCode": self.fixed_mat_code, "matName": self.fixed_mat_name, "id": self.fixed_mat_id}
            ...
            if isinstance(out5, dict) and out5.get("id"):
                ...
                uom_info = {"uomDesc": "件", "name": "件", "id": str(uom_id)}
            ...
            if isinstance(out6, dict) and out6.get("id"):
                ...
                curr_info = {"name": "人民币", "id": str(curr_id), "currName": "人民币"}
            ...
            price_adj_item = {
                ...
                "out5": uom_info.copy()
                if isinstance(uom_info, dict)
                else {"uomDesc": "件", "name": "件", "id": str(uom_id)},
                "out6": curr_info.copy()
                if isinstance(curr_info, dict)
                else {"name": "人民币", "id": str(curr_id), "currName": "人民币"},
            ...
            price_adj_item["outNew5"] = {"id": uom_id}
            price_adj_item["outNew6"] = {"id": curr_id}
            ...
            try:
                ...
                if price_adj_info:
                    ...
                    self.assert_util.assert_by_operator(
                        status, "in", ["INEFFECT", "EFFECT", "已生效"], message="价格维护单提交后状态应为已生效"
                    )
                    a.text(f"价格维护单提交成功，状态: {status}", "价格维护结果")
                    a.text(f"价格已从 {original_price} 更新为 {new_price}", "价格更新确认")
                else:
                    a.text("价格维护单提交成功，但无法查询状态", "价格维护结果")
            except Exception as e:
                ...
                if so_items:
                    ...
                    if found_item:
                        ...
                        if price_to_check is not None:
                            ...
                            a.text(
                                f"价格验证成功：订单中物料价格为 {price_to_check}，与维护后的价格 {expected_price} 一致",
                                "订单价格验证",
                            )
                        else:
