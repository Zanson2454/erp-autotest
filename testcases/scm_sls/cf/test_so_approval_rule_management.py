...
class TestSoApprovalRuleManagement(SlsBase):
    ...
    def teardown_class(cls):
        ...
        try:
            cls.db.delete(
                table="gen_sls_so_approval_rule_cf",
                where="approval_rule_code like %s",
                params=["AT_%"]
            )
    ...
    def test_so_approval_rule_export_task(self):
        ...
        try:
            ...
            set_dict = {
                "serviceKey": "SCM_SLS$SLS_SO_APPROVAL_RULE_CF_API_GEI_TASK_EXPORT_DIRECT_POST",
                "params": {
                    "taskName": f"销售审单规则-自动化测试-{self.mock_util.get_timestamp()}-导出",
                    "multiSheetConfig": [
                        {
