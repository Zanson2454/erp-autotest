...
class TestRebateUse(SlsBase):
    ...
    def test_rebate_policy_use_in_sales_order(self):
        """测试返利政策在销售订单中的使用"""
        try:
            # 1. 清理旧的返利政策
            ...
            rebate_policy = self.create_and_approve_rebate_policy(
                policy_name="自动化返利政策_使用测试", policy_code="AT_REB_USE_001"
            )
            ...
            filtered_detail_params = ParamUtil.filter_post_body_fields(detail_params, ["id"], ["params", "request"])
            ...
            for attempt in range(5):
                detail_response, _ = self.standard_api_call(
                    api_key="销售订单页面完整查询",
                    set_dict=(
                        filtered_detail_params.get("params", {})
                        if isinstance(filtered_detail_params, dict)
                        else filtered_detail_params
                    ),
                    store_id_as=None,
            ...
            # 设置提交参数（移除平台噪音字段）
            filtered_submit_params = ParamUtil.filter_post_body_fields(submit_params, ["params"], [])
            ...
            submit_response, _ = self.standard_api_call(
                ...
                set_dict=(
                    filtered_submit_params.get("params", {})
                    if isinstance(filtered_submit_params, dict)
                    else filtered_submit_params
                ),
                store_id_as=None,
                use_param_util=False,
                param_path=["params"],
            ...
            if records:
                ...
                # 断言验证流水金额与返利金额一致
                assert (
                    abs(float(flow_amount) - float(rebate_amount)) < 0.01
                ), f"返利账户流水金额 {flow_amount} 与订单返利金额 {rebate_amount} 不一致"
