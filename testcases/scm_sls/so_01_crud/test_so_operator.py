...
from utils.exception_util import handle_class_method_exception, safe_api_call
...
class TestSalesOrderOperator(SlsBase):
    ...
    def _query_draft_orders_from_db(self) -> Dict[str, Any]:
        ...
        order = result[0]
        self.test_data = {"so_id": str(order["id"]), "so_code": order["so_code"], "so_status": order["so_status"]}
    ...
    def _query_approving_orders_from_db(self) -> Dict[str, Any]:
        ...
        order = result[0]
        return {"so_id": str(order["id"]), "so_code": order["so_code"], "so_status": order["so_status"]}

    @case_decorator(
        story="销售订单操作",
        title="查询销售订单详情",
        description="测试查询销售订单详情",
        severity="critical",
        file_level_order=1,
    )
    ...
    def test_query_order_detail(self):
        ...
        try:
            ...
            # 构建请求体（移除平台噪音字段：sceneKey, viewKey, serviceKey）
            data = {
                "params": {"request": {"id": str(self.order_id)}},
            }
    ...
    @case_decorator(
        story="销售订单操作",
        title="销售订单编辑",
        description="测试销售订单编辑",
        severity="critical",
        file_level_order=2,
    )
    @safe_api_call(error_message="销售订单编辑失败")
    def test_sales_order_edit(self):
        """测试编辑销售订单"""
        # 查询草稿态订单
        draft_order = self._query_draft_orders_from_db()
        if not draft_order:
        ...
        # 构造请求数据（移除平台噪音字段：sceneKey, viewKey, serviceKey）
        request_data = {
            "params": {"request": {"id": str(self.order_id)}},
        }
        ...
        # 编辑订单 - 使用配置文件中的API
        api_info = self.apis["销售订单页面完整查询"]
        api_info["path"] if isinstance(api_info, dict) else api_info
        data = request_data
                    ...
                    so_items.append(
                        {
                            ...
                            "soItemTransferQty": float(item["so_item_transfer_qty"])
                            if item.get("so_item_transfer_qty")
                            else 0,
                            "soItemPrice": float(item["so_item_price"]) if item.get("so_item_price") else 0,
                            "uomSlsId": {"id": item["uom_sls_id"]},
                            "uomBaseId": {"id": item["uom_base_id"]}
                            if item.get("uom_base_id")
                            else {"id": item["uom_sls_id"]},
    ...
    @case_decorator(
        story="销售订单操作",
        title="销售订单编辑提交",
        description="测试销售订单编辑提交",
        severity="critical",
        file_level_order=3,
    )
    @safe_api_call(error_message="销售订单编辑提交失败")
    def test_submit_sales_order_edit(self):
        """测试销售订单编辑提交"""
        try:
            # 准备订单数据（避免测试方法之间直接调用）
            if not self.so_data:
            ...
            # 构造请求数据
            request_data = {"params": {"request": {**self.so_data, "syncSubmit": True}}}

            # 发送请求 - 使用保存服务
            api_info = self.apis["SLS-销售订单-保存服务"]
            api_info["path"] if isinstance(api_info, dict) else api_info

            # 构造请求数据（移除平台噪音字段：sceneKey, viewKey, viewTitle, buttonKey, buttonName, serviceKey）
            ...
            assert (
                new_so_code == self.so_data["soCode"]
            ), f"订单号不匹配: 期望={self.so_data['soCode']}, 实际={new_so_code}"

            so_status = self.query_service.query(
                f"select id,so_code,so_status from sls_so_head_tr where so_code='{new_so_code}'"
            )
            # 订单保存后进入审批状态是正常的业务流程
    ...
    @case_decorator(
        story="销售订单操作",
        title="销售订单列表提交",
        description="测试销售订单列表提交",
        severity="critical",
        file_level_order=4,
    )
    @safe_api_call(error_message="销售订单列表提交失败")
    def test_manual_submit_sales_order(self):
        """测试销售订单列表提交"""
        # 先创建一个有订单行的草稿态订单
                        ...
                        so_items.append(
                            {
                                ...
                                "soItemTypeId": {"id": item["so_item_type_id"]}
                                if item.get("so_item_type_id")
                                else None,
                                "soSchlDelDate": so_schl_del_date,
                                "invOrgId": {"id": item["inv_org_id"]},
                                "invLocId": {"id": item["inv_loc_id"]},
        ...
        # 验证订单状态
        so_status = self.query_service.query(
            f"select id,so_code,so_status from sls_so_head_tr where id={self.order_id}"
        )
        assert so_status[0]["so_status"] in [
            "APPROVING",
            "EFFECT",
        ], f"销售订单提交失败，订单状态: {so_status[0]['so_status']}"

    @case_decorator(
        story="销售订单操作",
        title="取消提交销售订单",
        description="测试取消提交销售订单",
        severity="critical",
        file_level_order=5,
    )
    @safe_api_call(error_message="取消提交销售订单失败")
    def test_cancel_submit_sales_order(self):
        """测试取消提交销售订单"""
        # 1. 创建一个销售订单并提交
        ...
        # 获取订单号和状态
        order_info = self.query_service.query(
            "SELECT so_code, so_status FROM sls_so_head_tr WHERE id = %s", (self.order_id,)
        )
        if not order_info:
        ...
        if not result.get("success", False):
            ...
            if "tr.so.relate.dn.not.discarded" in error_code or "未作废" in error_msg:
                ...
                if dn_ids:
                    ...
                    assert result.get(
                        "success", False
                    ), f"取消提交销售订单失败（已作废交货单后重试）: {result.get('err', {}).get('msg', '未知错误')}"
                else:
                    # 如果没有查询到交货单，直接抛出原始错误
        ...
        # 验证订单状态
        so_status = self.query_service.query(
            "SELECT id,so_code,so_status FROM sls_so_head_tr WHERE id = %s", (self.order_id,)
        )
    ...
    @case_decorator(
        story="销售订单操作",
        title="作废销售订单",
        description="测试作废销售订单",
        severity="critical",
        file_level_order=6,
    )
    @safe_api_call(error_message="作废销售订单失败")
    def test_repeal_sales_order(self):
        """测试作废销售订单"""
        ...
        if not has_schedule:
            ...
            pytest.fail(f"订单 {self.order_id} 没有发货计划行，作废订单需要发货计划行")
        ...
        # 构造请求体（移除平台噪音字段：sceneKey, viewKey, viewTitle, buttonKey, buttonName, appId, teamId, serviceKey）
        request_body = {
            "params": {"request": {"id": self.order_id}},
        }
    ...
    @case_decorator(
        story="销售订单操作",
        title="冻结销售订单",
        description="测试冻结销售订单",
        severity="critical",
        file_level_order=7,
    )
    @safe_api_call(error_message="冻结销售订单失败")
    def test_freeze_sales_order(self):
        """测试冻结销售订单"""
        try:
            ...
            # 验证订单状态
            so_status = self.query_service.query(
                f"select id,so_code,so_status,freeze_type from sls_so_head_tr where id={self.order_id}"
            )
    ...
    @case_decorator(
        story="销售订单操作",
        title="复制销售订单",
        description="测试复制销售订单",
        severity="critical",
        file_level_order=8,
    )
    @safe_api_call(error_message="复制销售订单失败")
    def test_copy_sales_order(self):
        """测试复制销售订单"""
        try:
            # 优先查询生效态订单，如果查询不到则创建新订单（避免并行执行时的数据竞争）
            effective_order = self._query_effective_orders_from_db()
            if not effective_order:
                ...
                # 查询订单号
                order_info = self.query_service.query(
                    "SELECT so_code FROM sls_so_head_tr WHERE id = %s", (self.order_id,)
                )
            ...
            # 构造请求数据
            request_data = {"params": {"request": {"id": str(self.order_id)}}}
    ...
    @case_decorator(
        story="销售订单操作",
        title="提交复制的销售订单",
        description="测试提交复制的销售订单",
        severity="critical",
        file_level_order=9,
    )
    @safe_api_call(error_message="提交复制的销售订单失败")
    def test_submit_copied_sales_order(self):
        """测试提交复制的销售订单"""
        try:
            ...
            assert result.get(
                "success", False
            ), f"提交复制的销售订单失败: {result.get('err', {}).get('msg', '未知错误')}"

            # 验证订单状态
            so_status = self.query_service.query(
                f"select id,so_code,so_status from sls_so_head_tr where id={copied_order_id}"
            )
            assert so_status[0]["so_status"] in [
                "APPROVING",
                "EFFECT",
            ], f"复制订单提交失败，订单状态: {so_status[0]['so_status']}"
...
if __name__ == "__main__":
    ...
    test.setup_class()
    test.test_query_order_detail()
    test.test_sales_order_edit()
    test.test_submit_sales_order_edit()
    test.test_manual_submit_sales_order()
    test.test_cancel_submit_sales_order()
    test.test_repeal_sales_order()
    test.test_freeze_sales_order()
    test.test_copy_sales_order()
    test.test_submit_copied_sales_order()
