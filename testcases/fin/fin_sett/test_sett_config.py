...
class TestSettConfig(BaseTest):
    ...
    def test_enable_sett_type(self):
        url=self.fin_path["结算类型-启用主数据服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
            ["id"],
        ...
        ParamUtil.set_request_params(data,set_dict)
        result=self.http.post(url,json=data,description=f"启用结算方式")
        self.assert_util.assert_response_success(result)
        a.json(data, "请求数据")
        a.json(result, "响应数据")
    ...
    def test_add_sett_doc_type(self):
        now_str = datetime.now().strftime("%Y%m%d%H%M%S")
        url=self.fin_path["结算单类型定义表-保存数据服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
        ...
        ParamUtil.set_request_params(data,set_dict)
        result=self.http.post(url,json=data,description=f"新增结算单据类型")
        self.assert_util.assert_response_success(result)
    ...
    def test_add_sett_item_type(self):
        url=self.fin_path["结算行项类型定义表-保存数据服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
    ...
    def get_sett_sds_head_invoke_code(self):
        url=self.fin_path["结算单汇单策略头表-调用取号规则服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
            [],
            ["params","request"])
        result=self.http.post(url,json=data,description=f"获取汇单策略调用取号规则")
    ...
    def test_add_sett_strategy(self):
        url=self.fin_path["SETT-SDS-汇单策略保存服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
        ...
        ParamUtil.set_request_params(data,set_dict)
        result=self.http.post(url,json=data,description=f"新增汇单策略")
        self.assert_util.assert_response_success(result)
    ...
    def get_sett_rule_link_item_code(self):
        url=self.fin_path["结算单汇单维度配置行表-调用取号规则服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
            [],
            ["params","request"])
        result=self.http.post(url,json=data,description=f"获取汇单维度配置行表调用取号规则")
        return result.get("data",{}).get("data",{})
    
    def get_sett_rule_scope_item_code(self):
        url=self.fin_path["结算单汇单范围配置行表-调用取号规则服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
            [],
            ["params","request"])
        result=self.http.post(url,json=data,description=f"获取汇单范围配置行表调用取号规则")
        return result.get("data",{}).get("data",{})
    
    
    def get_sett_rule_invoke_code(self):
        """获取汇单规则调用取号规则"""
        url=self.fin_path["结算单汇单配置头表-调用取号规则服务"]["path"]
        data=self.fin_params.get(url,{})
        data=ParamUtil.filter_post_body_fields(
            data,
            [],
            ["params","request"])
        result=self.http.post(url,json=data,description=f"获取汇单规则调用取号规则")
        return result.get("data",{}).get("data",{})
    ...
    def test_add_sett_rule(self):
        url=self.fin_path["结算单-汇单规则保存服务"]["path"]
        data=self.fin_params.get(url,{})
        sql="""
        ...
        ParamUtil.set_request_params(data,set_dict)
        result=self.http.post(url,json=data,description=f"新增汇单规则")
        self.assert_util.assert_response_success(result)
        a.json(data, "请求数据")
        a.json(result, "响应数据")
