...
WHERE NOT EXISTS (
    ...
    WHERE type_code = 'ISSUE_FORWARD' AND deleted = 0
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM prd_issue_type_cf 
    WHERE type_code = 'ISSUE_REVERSE' AND deleted = 0
);
...
WHERE NOT EXISTS (
    ...
    WHERE name = 'DEFAULT_RULE' AND deleted = 0
);
...
SELECT 
    'INV_ORG', 1, 0,
    0, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP,
    0, 0, 0, (SELECT id FROM prd_issue_rule_cf WHERE name = 'DEFAULT_RULE' AND deleted = 0)
WHERE NOT EXISTS (
    SELECT 1 FROM prd_issue_rule_item_cf pcsi
    JOIN prd_issue_rule_cf pcsh ON pcsi.prd_issue_rule_cf_id = pcsh.id
    WHERE pcsh.name = 'DEFAULT_RULE' AND pcsi.item = 'INV_ORG'
