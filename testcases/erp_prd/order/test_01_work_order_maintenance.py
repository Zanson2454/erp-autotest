    ...
    def test_find_prd_version(self):
        ...
        try:
            ...
            with a.step("4. 保存生产版本数据"):
                ...
                TestPrdOrder.prd_order_info.update({"prd_version_info": version_data})
    ...
    def test_find_routings_items(self):
        ...
        try:
            ...
            with a.step("3. 验证响应结果"):
                ...
                # 记录验证结果
                a.text(f"工艺路线数量: {len(routings_items)}\n验证结果: 成功", "验证结果")

            with a.step("4. 保存工艺路线数据"):
                # 保存工艺路线数据到类变量，供后续测试用例使用
                TestPrdOrder.prd_order_info.update(
                    {"routing_items": routings_items, "prdVrsId": filtered_params["params"]["request"]["prdVrsId"]}
                )
    ...
    def test_find_bom_items(self):
        ...
        try:
            ...
            with a.step("3. 验证响应结果"):
                ...
                # 记录验证结果
                a.text(f"BOM清单数量: {len(bom_items)}\n验证结果: 成功", "验证结果")

            with a.step("4. 保存BOM数据"):
                # 保存BOM数据到类变量，供后续测试用例使用
                TestPrdOrder.prd_order_info.update({"bom_items": bom_items})
    ...
    def test_create_prd_order(self):
        ...
        try:
            ...
            with a.step("3. 验证响应结果"):
                ...
                # 记录验证结果
                a.text(f"生产订单ID: {wo_id}\n生产订单编号: {wo_code}\n验证结果: 成功", "验证结果")

            with a.step("4. 保存生产订单信息"):
                # 保存生产订单信息到类变量，供后续测试用例使用
                TestPrdOrder.prd_order_info.update({"wo_id": wo_id, "wo_code": wo_code})
