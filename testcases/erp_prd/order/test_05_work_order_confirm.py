...
class TestWorkOrderConfirm(PrdBaseTest):
    ...
    def init(self):
        ...
        self.logger.info(
            f"获取到生产订单ID: {self.confirm_info['id']}, 编号: {self.confirm_info['wo_code']}, 状态: {self.confirm_info['status']}"
        )
        ...
        try:
            ...
            with a.step(f"查询工序报工列表-{self.confirm_info['wo_code']}"):
                ...
                self.confirm_info.update(
                    {"total": result.get("data", {}).get("total", 0), "records": result.get("data", {}).get("data", [])}
                )
    ...
    def test_batch_confirm_routings(self):
        ...
        try:
            with a.step(f"批量确认工序-{self.confirm_info['wo_code']}"):
                ...
                # 设置请求参数
                filtered_params = {"params": {"request": routing_ids}}
    ...
    def test_delivery_confirm_batch(self):
        ...
        try:
            with a.step(f"批量报工确认-{self.confirm_info['wo_code']}"):
                ...
                # 设置请求参数
                filtered_params = {"params": {"request": request_list}}
                ...
                for confirm, request in zip(confirm_data, request_list):
                    # 验证failReason为空
                    assert (
                        confirm.get("failReason") == ""
                    ), f"报工单{confirm.get('confirmCode')}报工失败: {confirm.get('failReason')}"
        ...
        try:
            with a.step(f"查询送货单状态-{self.confirm_info['wo_code']}"):
                ...
                for dn_info in dn_codes:
                    ...
                    self.logger.info(
                        f"送货单{dn_code}状态: delStatus={del_status}, bizStatus={biz_status}, btClass={bt_class}"
                    )
                    ...
                    assert (
                        head_record.get("doc_code") == self.confirm_info["wo_code"]
                    ), f"送货单{dn_code}关联的生产订单不匹配"

                    # 验证送货单状态
                    self.logger.info(
                        f"送货单{dn_code}详细信息: delStatus={del_status}, bizStatus={biz_status}, btClass={bt_class}"
                    )
                    self.logger.info(
                        f"送货单{dn_code}是否入库工序: {dn_info['isInboundOperation']}, 是否有物料消耗: {dn_info['hasMaterialConsumption']}"
                    )
                    ...
                    if dn_info["isInboundOperation"]:
                        # 入库工序生成的送货单需要手工过账
                        assert (
                            biz_status == "WAIT_POST"
                        ), f"入库工序送货单{dn_code}业务状态应该是WAIT_POST，当前状态: {biz_status}"
                        self.logger.info(f"✓ 入库工序送货单{dn_code}状态正确: 需要手工过账 (WAIT_POST)")
                    elif dn_info["hasMaterialConsumption"]:
                        # 物料消耗的送货单是自动过账
                        assert (
                            biz_status == "POSTED"
                        ), f"物料消耗送货单{dn_code}业务状态应该是POSTED，当前状态: {biz_status}"
                    ...
                    if dn_info["hasMaterialConsumption"]:
                        ...
                        for item in item_results:
                            ...
                            assert (
                                plan_del_qty > 0
                            ), f"送货单{dn_code}行项目计划交货数量应该大于0，当前值: {plan_del_qty}"
                            ...
                            # 验证行项目状态
                            assert (
                                item_status == "INEFFECT"
                            ), f"送货单{dn_code}行项目状态应该是INEFFECT，当前状态: {item_status}"

                            # 对于自动过账的物料消耗，行项目业务状态应该是POSTED
                            if biz_status == "POSTED":
                                assert (
                                    item_biz_status == "POSTED"
                                ), f"自动过账送货单{dn_code}行项目业务状态应该是POSTED，当前状态: {item_biz_status}"
                                # 自动过账的情况下，实际交货数量应该等于计划数量
                                assert (
                                    real_del_qty == plan_del_qty
                                ), f"自动过账送货单{dn_code}实际交货数量应该等于计划数量，计划: {plan_del_qty}, 实际: {real_del_qty}"
                                self.logger.info(
                                    f"✓ 物料消耗行项目自动过账正确: 计划数量={plan_del_qty}, 实际数量={real_del_qty}"
                                )

                            self.logger.info(
                                f"送货单{dn_code}行项目详情: 物料ID={item.get('mat_id')}, 计划数量={plan_del_qty}, 实际数量={real_del_qty}, 状态={item_status}, 业务状态={item_biz_status}"
                            )
                ...
                summary = {
                    ...
                    "material_consumption_dn_count": len(
                        [dn for dn in dn_status_results if dn["hasMaterialConsumption"]]
                    ),
                ...
                self.logger.info(
                    f"送货单状态验证完成: 共{summary['total_dn_count']}个送货单，"
        ...
        try:
            with a.step("查询待过账入库工序送货单详情"):
                ...
                for head_record in head_results:
                    ...
                    post_detail_results.append({"dnCode": dn_code, "detail": detail, "isInboundOperation": True})
                ...
                # 汇总查询结果
                summary = {"total_inbound_dn_count": len(post_detail_results), "verification_passed": True}
        ...
        try:
            with a.step("执行送货单过账操作"):
                ...
                for detail_result in post_detail_results:
                    ...
                    head_record = head_results[0]
                    assert (
                        head_record["biz_status"] == "POSTED"
                    ), f"送货单{dn_code}过账后业务状态应为POSTED，实际为{head_record['biz_status']}"
                    assert (
                        head_record["del_status"] == "INEFFECT"
                    ), f"送货单{dn_code}过账后单据状态应为INEFFECT，实际为{head_record['del_status']}"
                    ...
                    for item in item_results:
                        assert (
                            item["biz_status"] == "POSTED"
                        ), f"送货单{dn_code}行项目业务状态应为POSTED，实际为{item['biz_status']}"
                        assert (
                            item["real_del_qty"] == item["plan_del_qty"]
                        ), f"送货单{dn_code}行项目实际交货数量应等于计划数量，实际为{item['real_del_qty']}，计划为{item['plan_del_qty']}"
                    ...
                    for mvm_item in mvm_results:
                        ...
                        assert (
                            float(mvm_item["mvm_qty"]) == float(dn_item["real_del_qty"])
                        ), f"移动凭证行{mvm_item['code']}数量{mvm_item['mvm_qty']}与入库单明细{dn_item['id']}实际交货数量{dn_item['real_del_qty']}不一致"
                        # 其他原有校验
                        assert mvm_item["mvm_uom_id"] is not None, f"移动凭证行 {mvm_item['code']} 单位ID不能为空"
                ...
                self.logger.info(
                    f"送货单过账操作完成: 共{summary['total_dn_count']}个送货单，"
