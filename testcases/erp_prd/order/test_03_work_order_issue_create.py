...
class TestPrdOrderIssueCreate(PrdBaseTest):
    ...
    def test_get_issue_rule_render_default(self):
        try:
            ...
            with a.step("4. 保存接口返回数据"):
                TestPrdOrderIssueCreate.rule_render_info.update({"response": result})
                    ...
                    "params": {
                        "request": {
                            ...
                            "issueRule": {
                                ...
                                "issueRule": {"code": "aaa", "name": "默认分单规则", "defaultValue": True},
                            },
                        }
                    }
                }
    ...
    def test_submit_issue_orders(self):
        ...
        try:
            ...
            with a.step("4. 保存提交结果数据"):
                # 保存提交结果信息到类变量
                self.issue_info.update({"submit_result": response_data})

                self.logger.info("领料单提交成功")
    ...
    def test_verify_issue_related_documents(self):
        ...
        try:
            with a.step("1. 准备请求参数"):
                # 获取API路径
                api_path = self.get_api_path("分页查询生产领料单") + "?tmodule=ERP_PRD"
                ...
                for attempt in range(max_retries):
                    ...
                    if all_posted:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            a.text("达到最大重试次数，使用当前状态数据进行验证", "验证结果")

                a.text(f"查询到 {len(data_list)} 条领料行项目数据", "验证结果")

            with a.step("3. 验证领料单数据"):
                # 定义必要字段列表
                required_fields = ["id", "prdOrderHeadId", "matId", "planQty", "issuedQty", "status", "dnCode"]
                ...
                for index, item in enumerate(data_list, 1):
                    ...
                    if current_status not in ["POSTED", "SUBMITTED"]:
                        raise AssertionError(
                            f"第 {index} 条数据状态不正确,期望:POSTED或SUBMITTED,实际:{current_status}"
                        )
                    ...
                    assert (
                        float(item["issuedQty"]) == float(item["planQty"])
                    ...
                    assert (
                        item["dnCode"] is not None and item["dnCode"].strip() != ""
                    ), f"第 {index} 条数据的交货单号为空"
            ...
            with a.step("4. 验证交货入库单数据"):
                ...
                for item in data_list:
                    ...
                    for attempt in range(max_retries):
                        ...
                        if len(delivery_head_result) > 0:
                            ...
                            if delivery_head["biz_status"] == "POSTED":
                                ...
                                self.logger.info(
                                    f"交货入库单 {item['dnCode']} 状态为 {delivery_head['biz_status']}，等待更新..."
                                )
                                if attempt < max_retries - 1:
                    ...
                    if current_status not in ["POSTED", "SUBMITTED"]:
                        raise AssertionError(
                            f"交货入库单 {item['dnCode']} 状态不正确,期望:POSTED或SUBMITTED,实际:{current_status}"
                        )
                    ...
                    for delivery_item in delivery_item_results:
                        ...
                        if current_item_status not in ["POSTED", "SUBMITTED"]:
                            raise AssertionError(
                                f"交货入库单 {item['dnCode']} 行项目 {delivery_item['dn_item_code']} 状态不正确,期望:POSTED或SUBMITTED,实际:{current_item_status}"
                            )

                        if current_item_status == "SUBMITTED":
                            self.logger.warning(
                                f"交货入库单 {item['dnCode']} 行项目 {delivery_item['dn_item_code']} 状态为SUBMITTED，可能还在处理中"
                            )

                        assert (
                            delivery_item["bt_class"] == "PRD_ISSUE"
                        ), f"交货入库单 {item['dnCode']} 行项目 {delivery_item['dn_item_code']} 业务类型不正确,期望:PRD_ISSUE,实际:{delivery_item['bt_class']}"

                        # 验证数量
                        assert (
                            float(delivery_item["plan_del_qty"]) > 0
                        ...
                        # 验证关联信息
                        assert (
                            delivery_item["mat_code"] is not None and delivery_item["mat_code"].strip() != ""
                        ), f"交货入库单 {item['dnCode']} 行项目 {delivery_item['dn_item_code']} 物料编码为空"
                        assert (
                            delivery_item["doc_code"] is not None and delivery_item["doc_code"].strip() != ""
                        ), f"交货入库单 {item['dnCode']} 行项目 {delivery_item['dn_item_code']} 来源单据编号为空"
                        assert (
                            delivery_item["doc_item_code"] is not None and delivery_item["doc_item_code"].strip() != ""
                        ...
                        assert len(mat_result) > 0, f"未找到物料编码 {delivery_item['mat_code']} 对应的物料ID"
