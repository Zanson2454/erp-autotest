...
class TestPrdOrderSend(PrdBaseTest):
    ...
    def init(self):
        ...
        assert (
            order_info["confirm_status"] == "UNCONFIRMED"
        ), f"生产订单确认状态不是未确认状态，当前状态: {order_info['confirm_status']}"
        assert (
            order_info["delivered_status"] == "UNDELIVERED"
        ), f"生产订单交付状态不是未交付状态，当前状态: {order_info['delivered_status']}"

        # 保存生产订单信息
        self.prd_order_info.update(order_info)
        self.logger.info(
            f"获取到生产订单ID: {self.prd_order_info['id']}, 编号: {self.prd_order_info['wo_code']}, 状态: {self.prd_order_info['status']}"
        )
    ...
    def test_send_prd_order(self):
        ...
        try:
            ...
            with a.step("3. 验证响应结果"):
                ...
                # 验证下达结果
                assert (
                    response_data.get("succeed", 0) > 0
                ), f"生产订单下达失败: {response_data.get('items', [{}])[0].get('failReason', '未知原因')}"
            ...
            with a.step("4. 保存生产订单下达信息"):
                ...
                self.prd_order_info.update({"send_result": response_data})

                self.logger.info("生产订单下达成功")
