...
class TestRouting(PrdBaseTest):
    ...
    @case_decorator(
        story="工艺路线管理", title="查询BOM信息", description="查询BOM信息", severity="normal", file_level_order=1
    )
    def test_query_bom(self):
        """测试查询BOM信息"""
        try:
            with a.step("1. 准备查询参数"):
                ...
                # 设置请求参数
                filtered_params = ParamUtil.filter_post_body_fields(params, list(query_data.keys()), [])
            ...
            with a.step("3. 验证响应结果"):
                ...
                for bom in bom_list:
                    ...
                    bom.get("statusId", {}).get("id") if isinstance(bom.get("statusId"), dict) else None

                    if inv_org_id == self.test_org["id"] and mat_id == self.test_material["id"]:
                        matching_bom = bom
                        # 保存BOM相关信息到类变量
    ...
    @case_decorator(
        story="工艺路线管理", title="获取物料组件", description="获取物料组件", severity="normal", file_level_order=2
    )
    def test_get_material_components(self):
        """测试获取物料组件信息"""
        try:
            with a.step("1. 准备查询参数"):
                # 获取API配置
            ...
            with a.step("3. 验证响应结果"):
                ...
                for item in data_list:
                    if isinstance(item, dict) and "prdBomItemMdId" in item:
                        component = {"context": {}, "prdBomItemMdId": item["prdBomItemMdId"], "isBackflush": False}
    ...
    @case_decorator(
        story="工艺路线管理", title="创建工艺路线", description="创建工艺路线", severity="blocker", file_level_order=3
    )
    ...
    def test_get_routing_detail(self):
        """测试查询工艺路线详情"""
        try:
            ...
            with a.step("2. 准备查询详情参数"):
                ...
                # 准备请求数据
                query_data = {"params": {"request": {"id": routing_id}}}
            ...
            with a.step("4. 验证响应结果"):
                ...
                for i, (item, expected) in enumerate(zip(item_list, self.OPERATIONS)):
                    ...
                    assert (
                        item.get("controlKeyId", {}).get("id") == expected["controlKeyId"]["id"]
                    ), f"工序{i+1}控制码不匹配"
    ...
    def test_assign_operation(self):
        ...
        try:
            with a.step("1. 准备保存数据"):
                ...
                # 使用test_get_routing_detail中获取的详情数据
                ...
                # 准备请求数据
                query_data = {"params": {"request": routing_detail}}
...
if __name__ == "__main__":
    ...
    test = TestRouting()
    test.setup_class()
    test.test_query_bom()  # 查询BOM信息
    test.test_get_material_components()  # 获取物料组件
    test.test_create_routing()  # 创建工艺路线
    test.test_get_routing_detail()  # 查询工艺路线详情
    test.test_assign_operation()  # 分配工序并保存
