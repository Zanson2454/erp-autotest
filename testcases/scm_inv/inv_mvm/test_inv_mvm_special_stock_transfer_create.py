...
@allure.epic("库存管理")
@allure.feature("移动凭证创建器-特殊库存转移")
class TestMobileVoucherSpecialStockTransferCreator(MobileVoucherCreator):
    """移动凭证创建器测试类 - 特殊库存转移专项"""
    ...
    @case_decorator(
        story="移动凭证创建器",
        title="测试特殊库存转移-跨仓位转移",
        description="验证从收货区转移到发货区的特殊库存转移移动凭证创建，包含完整的出入库仓位参数和特殊库存标识",
        severity="blocker",
        order=1,
        smoke=True,
        tags=["移动凭证", "特殊库存转移", "跨仓位"]
    )
    def test_create_special_stock_transfer_voucher_specific_warehouse(self):
        """特殊库存转移-指定完整仓位和批次测试"""
        try:
            ...
            # 准备转移参数
            custom_to_batch = f"SPECIAL_{self.mock_util.get_timestamp()}"
            send_wh_info = self.md_cache_data["org_info"]["inv_bin_send_md"][0]
            
            # 创建特殊库存转移移动凭证
            voucher_info = self.create_special_stock_transfer_voucher(
            ...
            # 验证特殊库存信息
            self.assert_util.assert_by_operator(voucher_info["spc_stk_type_id"], "!=", None, "特殊库存类型ID不能为空")
            self.assert_util.assert_by_operator(voucher_info["spc_stk_type_class"], "!=", None, "特殊库存分类不能为空")
            self.assert_util.assert_by_operator(voucher_info["spc_stk_type_class_name"], "!=", None, "特殊库存分类名称不能为空")
            ...
            a.text(f"特殊库存类型ID: {voucher_info['spc_stk_type_id']}", "特殊库存信息")
            a.text(f"特殊库存分类: {voucher_info['spc_stk_type_class_name']} ({voucher_info['spc_stk_type_class']})", "特殊库存信息")
