...
class ErpCondBaseTest(BaseTest):
    ...
    def setup_class(cls):
        ...
        cls.api_params = cls.yaml_util.read_yaml(cls.cond_api_params).get("api_params", {}) if Path(cls.cond_api_params).exists() else {}
        ...
        if Path(sql_config_path).exists():
            DataFactory.init_sql_cache(
                ...
                cache_key="cond_init_cache",  # 缓存key
                cache_dir="testdata/cache"  # 缓存目录
            )
            cls.cond_cache_data = CacheUtil.get('cond_init_cache')
        else:
            # 复用主数据缓存
            cls.cond_cache_data = cls.md_cache_data if hasattr(cls, 'md_cache_data') else {}
        
        # 设置路径参数和用户信息 - 针对ERP_COND模块
        cls.path_params = {"tmodule":"ERP_COND"}
...
if __name__ == "__main__":
    ErpCondBaseTest.setup_class()
    print(ErpCondBaseTest.nickname)
    # print(ErpCondBaseTest.cust_user_info)
