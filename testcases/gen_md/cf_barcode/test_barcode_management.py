...
@allure.epic("通用基础数据")
@allure.feature("条码管理")
class TestBarcodeManagement(GenMdBaseTest):
    """条码管理测试类"""
    ...
    def teardown_class(cls):
        ...
        try:
            # 使用SQL删除测试数据
            cls.db.delete(
                table="gen_barcode_md",
                where="barcode_code like %s",
                params=["AT_%"]
            )
    ...
    def test_save_barcode(self):
        ...
        try:
            ...
            barcode_name = f"条码管理_{self.mock_data.get_timestamp()}"
            ...
            barcode_id = response.get("data", {}).get("data", {})
