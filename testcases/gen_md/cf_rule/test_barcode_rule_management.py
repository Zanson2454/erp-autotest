...
@allure.epic("通用基础数据")
@allure.feature("条码规则管理")
class TestBarcode_RuleManagement(GenMdBaseTest):
    """条码规则管理测试类"""
    ...
    def setup_class(cls):
        ...
        cls.logger.info("条码规则管理测试类初始化完成")

    @classmethod
    def teardown_class(cls):
        """
        测试类结束后执行清理
        清理所有测试过程中创建的条码规则管理数据
        """
        try:
            # 使用SQL删除测试数据
            cls.db.delete(
                table="gen_barcode_rule_md",
                where="barcode_rule_code like %s",
                params=["AT_%"]
            )
    ...
    def test_save_barcode_rule(self):
        ...
        try:
            # 准备条码规则管理数据
            barcode_rule_code = self.mock_data.generate_unique_code(tag="Barcode_Rule")
            barcode_rule_name = f"条码规则管理_{self.mock_data.get_timestamp()}"
            ...
            filtered_params = ParamUtil.filter_post_body_fields(
                params,
                ["barcode_rule_code", "barcode_rule_name"],
                ["params", "request"]
            )
            set_dict = {
                "barcode_rule_code": barcode_rule_code,
                "barcode_rule_name": barcode_rule_name
            }
            ...
            barcode_rule_id = response.get("data", {}).get("data", {})
