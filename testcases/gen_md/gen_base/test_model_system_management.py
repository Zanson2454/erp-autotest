...
class TestModelSystemManagement(GenMdBaseTest):
    ...
    def test_query_model_system_by_ids(self):
        ...
        try:
            if not self.model_system_ids:
                self.test_query_model_system_page()

            # 如果分页查询没有返回数据，使用模拟ID
            if not self.model_system_ids:
                self.logger.warning("未获取到模型系统ID，使用模拟ID进行测试")
                self.model_system_ids = [1, 2, 3]
            ...
            set_dict = {"ids": self.model_system_ids}
            ParamUtil.set_request_params(filtered_params, set_dict)
            ...
            if data_list:
                self.assert_util.assert_by_operator(len(data_list), ">=", 1, "应该返回至少一条模型系统数据")
