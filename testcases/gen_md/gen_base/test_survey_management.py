...
class TestSurveyManagement(GenMdBaseTest):
    ...
    def bind_context(cls):
        ...
        if cls.md_cache_data:
            cls.cust_template_id = (
                cls.md_cache_data.get("dynamic_form_info", {})
                .get("dynamic_form_template_md", {})
                .get("cust_template", [])[0]
                .get("id")
            )
            cls.vend_template_id = (
                cls.md_cache_data.get("dynamic_form_info", {})
                .get("dynamic_form_template_md", {})
                .get("vend_template", [])[0]
                .get("id")
    ...
    def _create_survey_mission(self):
        ...
        set_dict = {
            ...
            "surveyMissionItem": [
                {"weight": 10, "template": {"id": self.cust_template_id}, "user": {"id": self.user_id}}
            ],
    ...
    def _release_survey_mission(self):
        ...
        response, _ = self.standard_api_call(
            api_key="GEN-评分任务-发布评分任务服务", set_dict=set_dict, fields_to_filter=["id"], store_id_as=None
        )
    ...
    def _query_survey_detail(self):
        ...
        response, _ = self.standard_api_call(
            api_key="GEN-评分详情-查询评分详情服务", set_dict=set_dict, fields_to_filter=["id"], store_id_as=None
        )
    ...
    def test_export_survey_mission(self):
        ...
        try:
            ...
            response, _ = self.standard_api_call(
                api_key="评分任务标准导出服务", set_dict=set_dict, fields_to_filter=["selectFields"]
            )
            self.assert_util.assert_response_data(response)
                    ...
                    "multiSheetConfig": [
                        {
                            ...
                            "sheetName": "评分任务",
                            "headerConfigList": [
                                {"name": "任务编码", "type": "TEXT", "field": "code"},
                                {"name": "任务名称", "type": "TEXT", "field": "name"},
                                {"name": "任务状态", "type": "TEXT", "field": "status"},
                            ],
                        }
                    ],
                    "queryData": {
                        ...
                        "params": {
                            "request": {"pageable": {"sortOrders": []}},
                            "selectFields": [{"field": "code"}, {"field": "name"}, {"field": "status"}],
                            "modelKey": "GEN_MD$gen_survey_mission_md",
                        },
                    },
                    "processConfig": {
    ...
    def test_export_survey_detail(self):
        ...
        try:
            ...
            response, _ = self.standard_api_call(
                api_key="评分详情标准导出服务", set_dict=set_dict, fields_to_filter=["selectFields"]
            )
            self.assert_util.assert_response_data(response)
                    ...
                    "multiSheetConfig": [
                        {
                            ...
                            "sheetName": "评分详情",
                            "headerConfigList": [
                                {"name": "详情编码", "type": "TEXT", "field": "code"},
                                {"name": "评分", "type": "NUMBER", "field": "score"},
                                {"name": "评价", "type": "TEXT", "field": "comment"},
                            ],
                        }
                    ],
                    "queryData": {
                        ...
                        "params": {
                            "request": {"pageable": {"sortOrders": []}},
                            "selectFields": [{"field": "code"}, {"field": "score"}, {"field": "comment"}],
                            "modelKey": "GEN_MD$gen_survey_detail_md",
                        },
                    },
                    "processConfig": {
