...
@allure.epic("通用基础数据")
@allure.feature("文本类型管理")
class TestText_TypeManagement(GenMdBaseTest):
    """文本类型管理测试类"""
    ...
    def setup_class(cls):
        ...
        cls.logger.info("文本类型管理测试类初始化完成")

    @classmethod
    def teardown_class(cls):
        """
        测试类结束后执行清理
        清理所有测试过程中创建的文本类型管理数据
        """
        try:
            # 使用SQL删除测试数据
            cls.db.delete(
                table="gen_text_type_md",
                where="text_type_code like %s",
                params=["AT_%"]
            )
    ...
    def test_save_text_type(self):
        ...
        try:
            # 准备文本类型管理数据
            text_type_code = self.mock_data.generate_unique_code(tag="Text_Type")
            text_type_name = f"文本类型管理_{self.mock_data.get_timestamp()}"
            ...
            text_type_id = response.get("data", {}).get("data", {})
