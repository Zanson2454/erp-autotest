...
class TestBomManagement(GenMdBaseTest):
    ...
    def test_save_bom(self):
        ...
        try:
            ...
            bom_name = f"BOM管理_{self.mock_data.get_timestamp()}"
            ...
            bom_id = response.get("data", {}).get("data", {})
    ...
    @case_decorator(
        story="BOM管理",
        title="测试查询BOM管理详情",
        description="验证BOM管理详情查询功能",
        severity="normal",
        order=3,
        smoke=True,
        tags=["BOM管理", "查询"]
    )
    def test_query_bom_detail(self):
        """
        查询BOM管理详情用例
        """
        try:
