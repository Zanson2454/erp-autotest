            ...
            with a.step("执行工单取消确认操作"):
                ...
                with a.step("获取取消确认前的数据"):
                    ...
                    pre_data = {
                        ...
                        "order": {
                            "confirm_status": pre_order["confirm_status"]
                        },
                        "confirm": {
                            "cancel_is": pre_confirm["cancel_is"],
                            "deleted": pre_confirm["deleted"]
                        },
                        "dn": {
                            "del_status": pre_dn["del_status"],
                            "biz_status": pre_dn["biz_status"]
                        }
                    }
                ...
                with a.step("获取并验证取消确认后的数据"):
                    ...
                    post_data = {
                        ...
                        "order": {
                            "confirm_status": post_order["confirm_status"]
                        },
                        "confirm": {
                            "cancel_is": post_confirm["cancel_is"],
                            "deleted": post_confirm["deleted"]
                        },
                        "dn": {
                            "del_status": post_dn["del_status"],
                            "biz_status": post_dn["biz_status"]
                        }
                    }
                    ...
                    with a.step("验证数据变化"):
                        # 验证工艺路线数据变化
                        assert post_data["routing"]["operation_qty"] == pre_data["routing"]["operation_qty"], "工序数量不应该改变"
                        assert post_data["routing"]["confirmed_qty"] == pre_data["routing"]["confirmed_qty"] - yield_qty, (
                            f"工艺路线确认数量扣减不正确，"
                        ...
                        expected_order_status = (
                            'UNCONFIRMED' if all(status == 'UNCONFIRMED' for status in routing_statuses)
                            else 'CONFIRMED' if all(status == 'CONFIRMED' for status in routing_statuses)
