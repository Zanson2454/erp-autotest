...
class TestWorkOrderConfirm(PrdBaseTest):
    ...
    def test_query_posted_inbound_dn_detail(self):
        """查询已过账入库工序送货单详情
        
        步骤：
        1. 使用已过账的入库工序送货单进行详情查询
        2. 验证送货单相关信息的完整性
        ...
        """
        try:
            with a.step(f"查询已过账入库工序送货单详情"):
                # 获取SCM模块的API配置
                api_path = self.get_cross_module_api_path("scm", "DEL-过账专属交货单详情服务")
                params, url = self.get_cross_module_api_params("scm", api_path)
                
                # 获取已过账的送货单信息
                posting_results = self.confirm_info.get("posting_results", [])
                assert posting_results, "没有找到已过账的送货单信息"
                
                post_detail_results = []
                for posting_result in posting_results:
                    dn_code = posting_result["dnCode"]
                    self.logger.info(f"查询已过账入库工序送货单{dn_code}的详情")
                    
                    # 查询送货单头表信息
                    head_sql = f"""
                        SELECT id, dn_code, del_status, biz_status, bt_class, doc_code,
                               inv_org_id, inv_loc_id, recv_addr_id, recv_contact_info,
                               recv_contact_phone, move_voucher_id, posting_date
                        FROM del_dn_head_tr 
                        WHERE dn_code = '{dn_code}' 
                        AND deleted = 0
                    """
                    head_results = self.db.query(head_sql)
                    assert head_results, f"未找到送货单{dn_code}的头表记录"
                    head_record = head_results[0]
                ...
                # 保存所有查询结果
                self.confirm_info["posted_dn_detail_results"] = post_detail_results
                ...
                for result in post_detail_results:
                    report_item = {
                        ...
                        "detail": {
                            ...
                            "bizStatus": result["detail"].get("bizStatus"),
                            "docCode": result["detail"].get("docCode"),
                            "moveVoucherId": result["detail"].get("moveVoucherId"),
                            "postingDate": result["detail"].get("postingDate")
                        }
                    }
                    report_data.append(report_item)
                
                a.json(report_data, "已过账入库工序送货单详情查询结果")
                
                # 汇总查询结果
                summary = {
                    "total_posted_dn_count": len(post_detail_results),
                    "verification_passed": True
                }
                a.json(summary, "已过账入库工序送货单查询结果汇总")
                
                self.logger.info(f"已过账入库工序送货单详情查询完成: 共{summary['total_posted_dn_count']}个送货单")
    ...
    def test_execute_dn_cancel_posting(self):
        ...
        """
        try:
            with a.step(f"执行送货单冲销操作"):
                # 获取SCM模块的API配置
                api_path = self.get_cross_module_api_path("scm", "DEL-交货单取消过账服务")
                params, url = self.get_cross_module_api_params("scm", api_path)
                
                # 获取已过账的送货单详情
                posted_dn_details = self.confirm_info.get("posted_dn_detail_results", [])
                assert posted_dn_details, "没有找到需要冲销的送货单详情"
                
                cancel_results = []
                for detail_result in posted_dn_details:
                    dn_code = detail_result["dnCode"]
                    detail = detail_result["detail"]
                    ...
                    filtered_params = {
                        "params": {
                            "request": {
                                ...
                                "delStatus": detail["delStatus"],
                                "delClass": "RECV",
                                "recvContactPhone": detail["recvContactPhone"],
                                "recvContactInfo": detail["recvContactInfo"],
                                "recvAddrId": detail["recvAddrId"],
                    ...
                    assert head_record["del_status"] == "INEFFECT", f"送货单{dn_code}冲销后单据状态应为INEFFECT，实际为{head_record['del_status']}"
                    ...
                    # 2.3 验证冲销行数量与原行数量一致
                    assert len(mvm_results) == len(original_mvm_results), \
                    ...
                    for mvm_item in mvm_results:
                        ...
                        assert float(mvm_item["mvm_qty"]) == float(original_item["mvm_qty"]), \
                ...
                # 保存所有冲销结果
                self.confirm_info["cancel_results"] = cancel_results
                ...
                for result in cancel_results:
                    report_item = {
                        "dnCode": result["dnCode"],
                        "cancelSuccess": result["result"].get("success", False)
                    }
                    report_data.append(report_item)
                
                a.json(report_data, "送货单冲销操作结果")
                
                # 汇总冲销结果
                successful_count = len([r for r in cancel_results if r["result"].get("success", False)])
                summary = {
                    "total_dn_count": len(cancel_results),
                    "successful_cancel_count": successful_count,
                    "cancel_success_rate": f"{successful_count}/{len(cancel_results)}"
                }
                a.json(summary, "冲销操作汇总")
                
                self.logger.info(f"送货单冲销操作完成: 共{summary['total_dn_count']}个送货单，"
                               f"成功{summary['successful_cancel_count']}个")
...
if __name__ == "__main__":
    ...
    test.test_query_posted_inbound_dn_detail() # 查询已过账入库工序送货单详情
